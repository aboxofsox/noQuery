class noQuery {
  static init() {
    ...
    Node.prototype.attr = function(attr, value) {
      ...
      if (!attr) throw new Error('You did not specify an attribute.')
      if (!value) return e.getAttribute(attr)
      else return e.setAttribute(attr, value)
    }
    ...
    NodeList.prototype.attr = function(attr, value) {
      ...
      this.each(i => {
        ...
        if (!attr) throw new Error('You did not specify an attribute or value')
        if (!value) a = e.getAttribute(attr)
